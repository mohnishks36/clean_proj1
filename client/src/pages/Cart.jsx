...
const getUserAddress = async () => {
  try {
    const { data } = await axios.get('/api/address/get'); // ⬅️ No changes needed here
    if (data.success) {
 ...
 const placeOrder = async () => {
  ...
  withCredentials: true   // ✅ this sends cookies (token) with request
});
      ...
      console.log("🟢 Online Payment selected. Sending request...");
  ...
  withCredentials: true   // ✅ this sends cookies (token) with request
    ...
    // You can add Online Payment logic here if needed
...
>
  {Array.from({ length: Math.max(9, cartItems[product._id]) }, (_, idx) => (
